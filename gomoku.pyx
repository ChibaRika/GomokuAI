...
cdef int[:,:] chessboard = np.zeros((x,y), dtype = int) #初始化棋盘 __=0 ○=-1 ●=1

cdef short int move = 0
...
cdef int windows():
    ...
    for i in range(10000): #下棋
        ...
        if move % 2 == 0 and chessboard[posx][posy] == 0: #判断○●和是否有棋子
            ...
            piece = Circle(Point(posx * 75 + 75, posy * 75 + 75),35)
            piece.setFill("black")
        ...
        newpiece = Polygon(Point(posx * 75 + 75, posy * 75 + 75), \
                           Point(posx * 75 + 75 + 35, posy * 75 + 75), \
                           Point(posx * 75 + 75, posy * 75 + 75 + 35))
        ...
        if move > 2:
            ...
            print("AI用时：","%0.4f"%(end-start),"s")
            
cdef int lw(int chess): #连五判断，●●●●●
    cdef short int amount = 0
...
cdef int hs(int chess): #活四判断，__●●●●__
    cdef short int amount = 0
...
cdef int ssa(int chess): #死四判断A，__●●●●○，不考虑边界
    cdef short int amount = 0
...
cdef int ssb(int chess): #死四判断B，●__●●●
    cdef short int amount = 0
...
cdef int ssc(int chess): #死四判断C，●●__●●
    cdef short int amount = 0
...
cdef int hsana(int chess): #活三判断A，__●●●__
    cdef short int amount = 0
...
cdef int hsanb(int chess): #活三判断B，__●__●●__，不考虑边界
    cdef short int amount = 0
...
cdef int ssana(int chess): #死三判断A，____●●●○，不考虑边界
    cdef short int amount = 0
...
cdef int ssanb(int chess): #死三判断B，__●__●●○，不考虑边界
    cdef short int amount = 0
...
cdef int ssanc(int chess): #死三判断C，__●●__●○，不考虑边界
    cdef short int amount = 0
...
cdef int ssand(int chess): #死三判断D，●____●●
    cdef short int amount = 0
...
cdef int ssane(int chess): #死三判断E，●__●__●
    cdef short int amount = 0
...
cdef int ssanf(int chess): #死三判断F，○__●●●__○，不考虑边界
    cdef short int amount = 0
...
cdef int hea(int chess): #活二判断A，____●●____
    cdef short int amount = 0
...
cdef int heb(int chess): #活二判断B，__●__●__
    cdef short int amount = 0
...
cdef int hec(int chess): #活二判断C，__●____●__
    cdef short int amount = 0
...
cdef int sea(int chess): #死二判断A，______●●○，不考虑边界
    cdef short int amount = 0
...
cdef int seb(int chess): #死二判断B，____●__●○，不考虑边界
    cdef short int amount = 0
...
cdef int sec(int chess): #死二判断C，__●____●○，不考虑边界
    cdef short int amount = 0
...
cdef int sed(int chess): #死二判断D，●______●
    cdef short int amount = 0
...
cdef int ai(int chess):
    global move,win,piece,newpiece
    cdef short int aix = 0
    cdef short int aiy = 0
    cdef short int aim = 0
    cdef short int ain = 0
    cdef short int air = 0
    cdef short int ais = 0
    cdef short int aia = 0
    cdef short int aib = 0
    
    cdef int[:] aix2 = np.zeros(9, dtype = int)
    cdef int[:] aiy2 = np.zeros(9, dtype = int)
    cdef int[:] aim2 = np.zeros(9, dtype = int)
    cdef int[:] ain2 = np.zeros(9, dtype = int)
    
    cdef int[:] aix3 = np.zeros(7, dtype = int)
    cdef int[:] aiy3 = np.zeros(7, dtype = int)
    cdef int[:] aim3 = np.zeros(7, dtype = int)
    cdef int[:] ain3 = np.zeros(7, dtype = int)
    cdef int[:] aip3 = np.zeros(7, dtype = int)
    cdef int[:] aiq3 = np.zeros(7, dtype = int)
    cdef int[:] air3 = np.zeros(7, dtype = int)
    cdef int[:] ais3 = np.zeros(7, dtype = int)
    
    cdef int[:] aix4 = np.zeros(5, dtype = int)
    cdef int[:] aiy4 = np.zeros(5, dtype = int)
    cdef int[:] aim4 = np.zeros(5, dtype = int)
    cdef int[:] ain4 = np.zeros(5, dtype = int)
    cdef int[:] aip4 = np.zeros(5, dtype = int)
    cdef int[:] aiq4 = np.zeros(5, dtype = int)
    cdef int[:] air4 = np.zeros(5, dtype = int)
    cdef int[:] ais4 = np.zeros(5, dtype = int)
    cdef int[:] aiu4 = np.zeros(5, dtype = int)
    cdef int[:] aiv4 = np.zeros(5, dtype = int)
    cdef int[:] aia4 = np.zeros(5, dtype = int)
    cdef int[:] aib4 = np.zeros(5, dtype = int)
    
    cdef int minscore = 100000000
    cdef int maxscore = -100000000
    cdef int score1 = 0
    cdef int[:] maxscoremin = np.array([100000000]*9, dtype = int)
    cdef int[:] maxscoremin2 = np.array([100000000]*7, dtype = int)
    cdef int[:] maxscoremin3 = np.array([100000000]*5, dtype = int)
    cdef int maxscoremin4 = 100000000
    cdef bint skip = 0
    cdef bint winmove = 0
    ...
    for i in range(x):
        ...
        for j in range(y):
            ...
            if chessboard[i][j] == 0:
                ...
                if skip == 0 and maxscore < np.min(maxscoremin):
                    aix = i
                    aiy = j
                    
                if skip == 0 and maxscore < np.max(maxscoremin):
        ...
        for p in range(x):
            ...
            for q in range(y):
                ...
                if chessboard[p][q] == 0:
                    ...
                    if skip == 0 and maxscore < np.min(maxscoremin2):
                        aix = aix2[o]
                        aiy = aiy2[o]
                        
                    if skip == 0 and maxscore < np.max(maxscoremin2):
        ...
        for u in range(x):
            ...
            for v in range(y):
                ...
                if chessboard[u][v] == 0:
                    ...
                    if skip == 0 and maxscore < np.min(maxscoremin3):
                        aix = aix3[t]
                        aiy = aiy3[t]
                        
                    if skip == 0 and maxscore < np.max(maxscoremin3):
        ...
        for d in range(x):
            ...
            for e in range(y):
                ...
                if chessboard[d][e] == 0:
                    ...
                    if skip == 0 and maxscore < maxscoremin4:
                        aix = aix4[c]
                        aiy = aiy4[c]
    ...
    piece = Circle(Point(aix * 75 + 75, aiy * 75 + 75),35)
    if chess == 1:
        piece.setFill("black")
    else:
        piece.setFill("white")
    ...
    newpiece = Polygon(Point(aix * 75 + 75, aiy * 75 + 75), \
                       Point(aix * 75 + 75 + 35, aiy * 75 + 75), \
                       Point(aix * 75 + 75, aiy * 75 + 75 + 35))
    newpiece.setOutline("red")
    newpiece.setFill("red")
    newpiece.draw(win)
